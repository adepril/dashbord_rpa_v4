...
export default function MergedRequestForm({
  ...
  const submitForm = async () => {
    ...
    try {
      // Préparation des données pour l'insertion dans la table Evolutions
      const evolutionData = {
      ...
      const dbResponse = await fetch('/api/sql', {
        method: 'POST',
        headers: {
      ...
      // Envoi de l'email de notification (optionnel)
      try {
        await fetch('/api/sql?table=Evolutions', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        console.log('Erreur lors de l\'envoi de l\'email de notification:', emailError);
        // On continue même si l'email échoue, car les données sont déjà enregistrées
      }
      ...
      if (error instanceof Response) {
        ...
        errorData = { error: 'Échec de l\'enregistrement de la demande' };
      }
      toast({
        title: 'Erreur d\'enregistrement',
        description: errorData.error || 'Échec de l\'enregistrement de la demande',
        variant: 'destructive',
        id: ''
      });
    }
